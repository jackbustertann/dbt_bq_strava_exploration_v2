...
moving_time_in_zones AS (
    ...
    JOIN activity_zones
        ON streams.activity_id = activity_zones.activity_id
            AND ((streams.heartrate_bpm > activity_zones.lower_bound OR activity_zones.lower_bound = 0) 
            AND (streams.heartrate_bpm <= activity_zones.upper_bound OR activity_zones.upper_bound = -1))
    group by 1, 2
),
